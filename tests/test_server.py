...
flask = pytest.importorskip(
    "flask", reason="Flask not installed (pip install distill-score[server])"
)
...
class TestScore:
    ...
    def test_score_no_json(self, client):
        resp = client.post("/score", data="not json", content_type="text/plain")
        assert resp.status_code == 400
    ...
    def test_score_with_highlights(self, client):
        ...
        has_highlights = any("highlights" in dim for dim in data["dimensions"].values())
